...
// Helper to check if a date is between two dates
const isDateInRange = (date: Date, from: Date | undefined, to: Date | undefined): boolean => {
...
function Calendar({
  ...
  const rangeStart = props.mode === 'range' && props.selected
    ? (props.selected as DateRange)?.from
    : undefined

  // Only show hover preview when we have a start date but no end date
  const showHoverPreview = props.mode === 'range' && rangeStart && !(props.selected as DateRange)?.to

  return (
    <HoverContext.Provider value={{ hoveredDate: showHoverPreview ? hoveredDate : undefined, rangeStart }}>
      <DayPicker
...
function CalendarDayButton({
  ...
  const isHoveredEnd = hoveredDate &&
    rangeStart &&
